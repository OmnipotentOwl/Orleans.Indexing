...
namespace Orleans.Indexing
{
    ...
    internal class ActiveHashIndexPartitionedPerSiloBucketImplGrainService/*<K, V>*/ : GrainService, IActiveHashIndexPartitionedPerSiloBucket/*<K, V> where V : IIndexableGrain*/
    {
            ...
            : base(GetGrainIdentity(siloIndexManager, grainInterfaceType, parentIndexName), siloIndexManager.Silo, siloIndexManager.LoggerFactory)
        {
            ...
            this._parentIndexName = parentIndexName;
        ...
        {
            this.logger.Trace($"ParentIndex {this._parentIndexName}: Started calling DirectApplyIndexUpdateBatch with the following parameters: isUnique = {isUnique}," +
                              $" siloAddress = {siloAddress}, iUpdates = {MemberUpdate.UpdatesToString(iUpdates.Value)}", isUnique, siloAddress);

            await Task.WhenAll(iUpdates.Value.Select(kvp => this.DirectApplyIndexUpdates(kvp.Key, kvp.Value, isUnique, idxMetaData, siloAddress)));
        ...
        private async Task DirectApplyIndexUpdates(IIndexableGrain g, IList<IMemberUpdate> updates, bool isUniqueIndex, IndexMetaData idxMetaData, SiloAddress siloAddress)
        {
            ...
            {
                await this.DirectApplyIndexUpdate(g, updt, isUniqueIndex, idxMetaData, siloAddress);
            }
        }
        ...
        private Task<bool> DirectApplyIndexUpdate(V updatedGrain, IMemberUpdate updt, bool isUniqueIndex, IndexMetaData idxMetaData, SiloAddress siloAddress)
            ...
            => Task.FromResult(HashIndexBucketUtils.UpdateBucketState(updatedGrain, updt, this.state, isUniqueIndex, idxMetaData));
        ...
        {
            this.logger.Trace("Streamed index lookup called for key = {key}", key);

            if (this.state.IndexStatus != IndexStatus.Available)
            {
                throw this.LogException($"ParentIndex {this._parentIndexName}: Index is not still available.", IndexingErrorCode.IndexingIndexIsNotReadyYet_GrainServiceBucket1);
            }
        ...
        {
            this.logger.Trace($"ParentIndex {this._parentIndexName}: Eager index lookup called for key = {key}");

            if (this.state.IndexStatus != IndexStatus.Available)
            {
                throw this.LogException($"ParentIndex {this._parentIndexName}: Index is not still available.", IndexingErrorCode.IndexingIndexIsNotReadyYet_GrainServiceBucket2);
            }
            var entryValues = (this.state.IndexMap.TryGetValue(key, out HashIndexSingleBucketEntry<V> entry) && !entry.IsTentative) ? entry.Values : [];
        ...
        public Task<V> LookupUniqueAsync(K key)
        {
            if (this.state.IndexStatus != IndexStatus.Available)
            {
                throw this.LogException($"ParentIndex {this._parentIndexName}: Index is not still available.", IndexingErrorCode.IndexingIndexIsNotReadyYet_GrainServiceBucket3);
            }
            if (this.state.IndexMap.TryGetValue(key, out HashIndexSingleBucketEntry<V> entry) && !entry.IsTentative)
            {
                return (entry.Values.Count == 1)
                    ? Task.FromResult(entry.Values.GetEnumerator().Current)
                    : throw this.LogException($"ParentIndex {this._parentIndexName}: There are {entry.Values.Count} values for the unique lookup key \"{key}\" on index \"{IndexUtils.GetIndexNameFromIndexGrain(this)}\".",
                                      IndexingErrorCode.IndexingIndexIsNotReadyYet_GrainServiceBucket4);
            }
            throw this.LogException($"ParentIndex {this._parentIndexName}The lookup key \"{key}\" does not exist on index \"{IndexUtils.GetIndexNameFromIndexGrain(this)}\".",
        ...
        public Task<bool> IsAvailable()
            => Task.FromResult(this.state.IndexStatus == IndexStatus.Available);
    }
}
